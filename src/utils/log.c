...
void lsp_log_init(void)
{
    ...
    s_log_ctx = lsp_malloc(sizeof(lsp_log_ctx_t));
    ...
    int line, const char* fmt, ...)
{
    ...
    lsp_log_t* msg = lsp_malloc(malloc_size);
    msg->type = type;
	...
	if (!g_tags.flags.initialized ||
        g_tags.flags.shutdown ||
        (int)type > g_tags.config.lsp_log_level)
	{
        lsp_free(msg);
		return;
	}
